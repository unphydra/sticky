...
const LikeButton = ({ trigger, liked, id }) => {
  const handleClick = () => {
    StickyApi.postLike(id).then(() => trigger((t) => !t));
  };
...
const IconBar = ({ trigger, liked, id }) => (
  <div
    style={{
      display: 'flex',
  ...
  >
    <LikeButton trigger={trigger} liked={liked} id={id}></LikeButton>
